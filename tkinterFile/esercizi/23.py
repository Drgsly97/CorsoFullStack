...
def Autenticazione():
    ...
    for i in listaconti:
        if i.id == e2.get() and i.username == e1.get():
            ...
            e3.pack()
            bt2= Button(master,text="Prelievo", command=lambda : prelevare(int(e3.get())+i.saldo))
            bt2.place(relx=0.2 ,rely=0.5 ,anchor=CENTER)
            bt2.pack()
            bt3=Button(master,text="Versamento",command= lambda:versamento())
            bt3.place(relx=0.4, rely=0.5, anchor=CENTER)
            bt3.pack()
            bt4=Button(master,text="Bonifico",command= lambda :bonofico())
            bt4.place(relx=0.6, rely=0.5,anchor=CENTER)
            bt4.pack()
            bt5=Button(master,text="Saldo",command= lambda :StampaSaldo())
            bt5.place(relx=0.8,rely=0.5, anchor=CENTER)
            bt5.pack()
